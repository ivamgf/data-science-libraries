            ...
            if sentence.find(".//webanno.custom.Judgmentsentity") is not None:
                ...
                if sentence_text not in sentences:
                    ...
                    for sent_idx, sent in enumerate(sentences_list[:5]):  # Select up to 5 sentences
                        ...
                        # Transpose input
                        concatenated_emb = np.transpose(concatenated_emb, (1, 0))  # Alteração na linha

                        # Generate example data
                        num_samples = 1
                        X = concatenated_emb.reshape((num_samples, 1, 120))  # Reshape the input data
                        y = tf.random.uniform((num_samples, num_classes))

                        # Create Bidirectional LSTM model
                        lstm_model = tf.keras.Sequential()
                        lstm_model.add(
                            tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(hidden_size, input_shape=(1, 120))))
